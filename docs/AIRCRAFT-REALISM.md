...
同系列货机为客机价格的95%，混合型为110%。现实参照不代表游戏美术复制厂商涂装、商标、注册号、照片或技术图纸；现役v5系列继续使用原创虚构名称和原创美术，DA40使用原创蓝白金涂装。

## 数值取舍

- DA40放在雨燕之前形成4,000→7,000金币的入门购买梯度。1客0货来自已确认的游戏容量；现实4座包含飞行员及非经营座位，不等于4个可售客位。
- DA40容量改装上限为0，避免1客机在同一机体上扩成10客；其他现役机型继续沿用存档9的最多9级扩舱，以免旧存档缩容或在途满载收入变化。
- 雨燕至极光的运行航程、速度、重量、容量与价格保持现值。当前v9严格校验会用目录重算在途时长、成本和收入，直接改值会拒绝合法旧存档；若要重平衡这些值，必须冻结v9核心、提升存档版本并迁移锁定合同。
