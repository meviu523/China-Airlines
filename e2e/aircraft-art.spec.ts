...
test('modern aircraft catalog uses left-facing unique revisioned bitmap art', async ({ page }) => {
  ...
  expect(sources.filter(source => /aircraft-(swift|heron|albatross|aurora)-(p|f|m)-exterior-v7\.png$/.test(source ?? ''))).toHaveLength(12);
  expect(sources).toContainEqual(expect.stringMatching(/aircraft-diamond-da40-exterior-v7\.png$/));
